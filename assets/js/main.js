...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Toggle volume slider visibility when clicking the icon
    volumeIcon.addEventListener('click', (e) => {
        e.stopPropagation();
        volumeSlider.classList.toggle('hidden');  // Show or hide the slider
    });

    // Hide volume slider when clicking outside of it
    document.addEventListener('click', (e) => {
        if (!volumeIcon.contains(e.target) && !volumeSlider.contains(e.target)) {
            volumeSlider.classList.add('hidden');  // Ensure the slider hides when clicking outside
        }
    });

    // Update audio player volume based on slider value
    volumeSlider.addEventListener('input', () => {
        audioPlayer.volume = volumeSlider.value;  // Set audio volume
    });

    // Set the volume slider to the current volume when page loads
